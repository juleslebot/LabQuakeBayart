...
names=['v     ','y_pos ','t_0   ','Gamma ','e']



### Optimization computation

args=np.array([500,0.003,0,1,invcot(500/C_s),0.01])
...
axfreq5 = plt.axes([0.25,0.35,0.65,0.03])


axfreq6 = plt.axes([0.25, 0.3, 0.65, 0.03])
e_slider = Slider(
    ax=axfreq6,
    label=r'$e$ [m]',
    valmin=bounds[0,4],
...
def update(val):
    args_temp=(v_slider.val, y_pos_slider.val,t_0_slider.val,Gamma_slider.val,2*invcot(v_slider.val/C_s)/np.pi,e_slider.val)


    #(2*/np.pi)

    axfreq5.clear()
    axfreq5.text(0,0,r"$\alpha=${}".format(invcot(v_slider.val/C_s)))
