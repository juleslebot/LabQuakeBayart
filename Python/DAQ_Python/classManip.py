...
class slowmon():
    ...
    def create_from_file(self, loc):
        ...
        self.gages = facq.convert(data_sm[gages_ch])
        self.time = time_sm

        self.eps_yy = self.gages[1::3]
        self.eps_xy= self.gages[2::3]
        self.eps_xx = self.gages[0::3]
...
class manip():
    def __init__(self,*args):
        arg = args[0]
        if os.path.isfile(loc+'data_manip.npy'):
            man_dict = np.load(loc+'data_manip.npy',allow_pickle=True).all()
    ...
    def create_from_file(self,loc_manip):
        self.loc = loc_manip
        #meta_data = np.load(loc_manip+'parameters.txt') # meta-donnee depuis un fichier
