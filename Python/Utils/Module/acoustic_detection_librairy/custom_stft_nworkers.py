...
def _spectral_helper(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    ### OLD CODE, MODIFIED YF2024, IMPROVING PERFORMANCE AND MEMORY USAGE
    ...
    if not same_data and x.shape[-1] != y.shape[-1]:
        if x.shape[-1] < y.shape[-1]:
            pad_shape = list(x.shape)
            pad_shape[-1] = y.shape[-1] - x.shape[-1]
            x = np.pad(x, [(0, 0)] * (x.ndim - 1) + [(0, pad_shape[-1])])
        else:
            pad_shape = list(y.shape)
            pad_shape[-1] = x.shape[-1] - y.shape[-1]
            y = np.pad(y, [(0, 0)] * (y.ndim - 1) + [(0, pad_shape[-1])])
    ...
    # Reduced Memory Usage with np.pad
    # Simplified Detrending using lambda functions
    # Optimized Conditional Checks to avoid redundancy
    # Grouped related operations to enhance readability
    ...
    if same_data and x.size == 0:
        return np.empty(x.shape), np.empty(x.shape), np.empty(x.shape)

    if not same_data and (x.size == 0 or y.size == 0):
        outshape = outershape + (min([x.shape[axis], y.shape[axis]]),)
        emptyout = np.moveaxis(np.empty(outshape), -1, axis)
        return emptyout, emptyout, emptyout
    ...
    if not detrend:
        ...
        detrend_func = lambda d: _signaltools.detrend(d, type=detrend, axis=-1)
    elif axis != -1:
        detrend_func = lambda d: np.moveaxis(detrend(np.moveaxis(d, -1, axis)), axis, -1)
    else:
        detrend_func = detrend
    ...
    if return_onesided:
        if np.iscomplexobj(x):
            ...
            sides = 'onesided'
            if not same_data and np.iscomplexobj(y):
                sides = 'twosided'
                warnings.warn('Input data is complex, switching to return_onesided=False')
    else:
        sides = 'twosided'
    ...
    if not same_data:
        result_y = _fft_helper(y, win, detrend_func, nperseg, noverlap, nfft, sides, nworkers=nworkers)
        result = np.conjugate(result) * result_y
    elif mode == 'psd':
        result = np.conjugate(result) * result
    ...
    time = np.arange(nperseg / 2, x.shape[-1] - nperseg / 2 + 1, nstep) / float(fs)
    if boundary is not None:
        time -= (nperseg / 2) / fs
