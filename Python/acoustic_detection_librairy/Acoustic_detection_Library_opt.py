...
from Python.Utils.Module.acoustic_detection_librairy.custom_stft_nworkers import my_stft
from Python.Utils.Module.fct_numpy import *
...
def Find_peaks(data_process, threshold=0.1, fs=10 ** 5):
    ...
    try:
        ...
        logging.warning('no peak detected')
        index_beg = np.array([])
        index_end = np.array([])
        peaks_final = np.array([])
        properties_final = np.array([])

    return index_beg, index_end, peaks_final, properties_final


def Find_peaks_easy(data_process, threshold=0.1,):
    """Given a processed data by STFT or CWT, this function finds the beginning and the end of events"""
    # data_process =(data_process-np.min(data_process))/(np.max(data_process)-np.min(data_process)) #Normalisation
    ### trouver idx events (sur STFT size)
    mask = data_process.copy()

    # average = data_process.copy()
    # average = np.cumsum(average, dtype=float)
    # average[n:] = average[100:] - average[:-100]
    # average = average[9:] / 10
    # mask[data_process < threshold and average < threshold] = 0
    mask[data_process < threshold] = 0

    index_beg, index_end = find_nonzero_connected_components(mask)

    peak_idx = np.zeros(index_beg.size, dtype=int)
    if index_beg.size != 0:
        for i in range(index_beg.size):
            where = np.where(data_process[index_beg[i]:index_end[i]] == np.max(data_process[index_beg[i]:index_end[i]]))[0]
            if where.size > 1:
                logging.warning('multiple max => on prend le premier')
                peak_idx[i] = where[0] + index_beg[i]
            else:
                peak_idx[i] = where + index_beg[i]

    if peak_idx.size != index_beg.size:
        logging.warning(('to many peak idx : {} wrt {}'.format(peak_idx.size, index_beg.size)))

    return index_beg, index_end, peak_idx
