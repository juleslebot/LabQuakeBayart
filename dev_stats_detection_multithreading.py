...
if __name__=='__main__':
    loc = 'C:/Users/Manips/Desktop/tmp/2024-06-25_full_manip/03/'
    filename = "Sampling03.tdms"


def main_traitement(loc,filename):
    locfile = loc + filename
    tdms_data = open_tdms_jules_acoustic(locfile)
    ...
    nseg = 512
    noverlap = nseg - 1
    Ncpu = 3  # max 5, sinon on a pas assez de ram...
    ...
    def detect_evnts(i, sub_idx, threshold, acq_fr, nseg, noverlap, wt=0.00005):
        if i < Ncpu:
            sleep(3.3*i)
        ini, end = sub_idx[i]
    ...
    array_to_run = np.arange(0,len(sub_idx))


    # Create a pool of worker threads
    with Pool(Ncpu) as pool:
        # Use partial to fix the arguments of process_event
        func = partial(detect_evnts, sub_idx=sub_idx, threshold=70, acq_fr=acq_fr, nseg=nseg, noverlap=noverlap)
        # Map the function to the range of indices using tqdm for progress bar
        dict = list(tqdm(pool.imap(func, array_to_run),total=len(array_to_run)))
    ...
    print("part {}\n".format(kkk))
    kkk+=1
    if not(os.path.exists(loc+'save_stats/')):
        os.mkdir(loc+'save_stats/')
    np.save(loc+'save_stats/pos_idx',pos_idx)
    np.save(loc+'save_stats/neg_idx',neg_idx)
    np.save(loc+'save_stats/peak_idx',peak_idx)
    np.save(loc+'save_stats/nrj_sum_signal',nrj_sum_signal)
    np.save(loc+'save_stats/nrj_sum_stft',nrj_sum_stft)
    np.save(loc+'save_stats/nrj_max_stft',nrj_max_stft)
    np.save(loc+'save_stats/dt',dt)

    if __name__ == '__main__':
        return start_time,steps,pos_idx,neg_idx,peak_idx,nrj_sum_signal,nrj_sum_stft,nrj_max_stft,dt,end_time

## Durées des events
if __name__=='__main__':
    start_time,steps,pos_idx,neg_idx,peak_idx,nrj_sum_signal,nrj_sum_stft,nrj_max_stft,dt,end_time = main_traitement(loc,filename)
    ...
    yname = '\Delta t_{btw}'
    fig, ax = plt.subplots(1)
    theta = 0.5 * dt_btw ** (-0.33) * np.exp(-dt_btw / 1.59)
    ax.plot(dt_btw,theta, '.', color='k')
